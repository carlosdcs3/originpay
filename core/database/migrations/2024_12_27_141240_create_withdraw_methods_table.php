...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('withdraw_methods', function (Blueprint $table) {
            ...
            $table->double('min_withdraw')->comment('Minimum withdrawal limit');
            $table->double('max_withdraw')->comment('Maximum withdrawal limit');
            $table->boolean('conversion_rate_live')->default(0)->comment('0 = fixed rate, 1 = live rate');
            $table->double('conversion_rate')->comment('Exchange rate');
