...
class VirtualCardRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (! $provider) {
            notifyEvs('error', 'Rede ou moeda selecionada não é suportada por nenhum provedor.');
        ...
        try {
            ...
            DB::commit();
            notifyEvs('success', 'A solicitação do seu cartão foi enviada para análise do administrador.');
        } catch (\Exception $e) {
            DB::rollBack();

            Log::error('Failed to create virtual card request.', [
                'user_id' => $user->id,
                'wallet_id' => $wallet->id,
                'cardholder_id' => $request->cardholder_id,
                'exception' => $e,
            ]);
            notifyEvs('error', 'Falha ao enviar a solicitação do cartão. Tente novamente.');
