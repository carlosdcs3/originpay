...
class KycSubmissionController extends Controller
{
    ...
    public function kycVerify()
    {
        $kycTemplates = KycTemplate::where('status', true)
            ->whereJsonContains('applicable_to', auth()->user()->role?->value ?? \App\Enums\UserRole::USER->value)
            ->get();

        return view('frontend.user.setting.kyc_verify-v2', compact('kycTemplates'));
    }
        ...
        try {
            ...
            foreach ($kycTemplate->fields as $index => $field) {
                ...
                if (isset($field['type']) && $field['type'] === 'file' && $request->hasFile("credentials.{$field['label']}")) {
                    ...
                    $path = "private/kyc/{$dateFolder}";
                    $originalName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
                    $fileName = now()->format('Ymd_His').'_'.\Illuminate\Support\Str::slug($originalName, '_').'_'.\Illuminate\Support\Str::random(4).'.' . $file->getClientOriginalExtension();
            ...
            // Sync user's kyc_status to PENDING (Em Análise) and set kyc_type
            $user->update([
                'kyc_status' => KycStatus::PENDING->value,
                'kyc_type' => stripos($kycTemplate->title, 'PJ') !== false || stripos($kycTemplate->title, 'Jurídica') !== false ? 'pj' : 'pf'
            ]);
