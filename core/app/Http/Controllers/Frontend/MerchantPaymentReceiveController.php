...
use App\Services\PaymentCheckoutPresentationService;
use Exception;
...
class MerchantPaymentReceiveController extends Controller
{
    public function __construct(
        protected PaymentCheckoutPresentationService $presentationService
    ) {}
    ...
    public function paymentCheckoutSigned(Request $request)
    {
        ...
        $environment = $this->presentationService->detectEnvironmentMode($transaction);
        $isSandbox = $environment->isSandbox();

        $data = $this->presentationService->buildCheckoutData($transaction, $merchant);
        ...
        if ($isSandbox) {
            ...
            Log::channel('payments')->info('Sandbox payment checkout accessed', [
    ...
    public function paymentCheckoutPublic(string $merchant, string $token)
    {
        ...
        $data = $this->presentationService->buildCheckoutData($transaction, $merchantData);

        // Add sandbox-specific handling
        if ($isSandbox) {
            // Log sandbox payment attempt
            Log::channel('payments')->info('Sandbox payment checkout (public) accessed', [
                'transaction_id' => $transaction->trx_id,
                'merchant_id' => $transaction->trx_data['merchant_id'],
    ...
    public function processPayment(Request $request)
    {
        ...
        app(\App\Contracts\Metrics\OperationalMetricsServiceInterface::class)->increment('checkout_started'); Log::channel('payments')->info('Payment processing started', [
        ...
        if ($isSandbox) {
            ...
            Log::channel('payments')->info('Sandbox gateway payment auto-completed', [
                'transaction_id' => $validated['trx_id'],
                'payment_method' => $validated['selected_method'],
                'environment' => $environment->value,
            ]);
        ...
        Log::channel('payments')->info('Payment gateway redirect', [
            'transaction_id' => $validated['trx_id'],
            'payment_method' => $validated['selected_method'],
            'provider' => $provider,
            'environment' => $environment->value,
    ...
    public function completePayment(Request $request): RedirectResponse
    {
        ...
        // Detect environment mode
        $environment = $this->presentationService->detectEnvironmentMode($merchantTransaction);
        $isSandbox = $environment->isSandbox();
        ...
        if ($isSandbox && $validated['wallet_id'] === '123456789' && $validated['password'] === 'demo123') {
            ...
            Log::channel('payments')->info('Sandbox demo wallet payment completed', [
    ...
    public function payWithAccount(Request $request): RedirectResponse
    {
        ...
        if ($isSandbox) {
            ...
            Log::channel('payments')->info('Sandbox login payment auto-completed', [
                'transaction_id' => $merchantTransaction->trx_id,
                'environment' => $environment->value,
            ]);
    ...
    public function walletPayment(Request $request, $token)
    {
        ...
        if ($isSandbox) {
            ...
            Log::channel('payments')->info('Sandbox wallet payment accessed', [
                'transaction_id' => $transaction->trx_id,
                'merchant_id' => $transaction->trx_data['merchant_id'],
                'amount' => $transaction->payable_amount,
                'currency' => $transaction->payable_currency,
    ...
    protected function handleFailure($transaction, string $errorMessage, ?string $description = null): RedirectResponse
    {
        ...
        app(\App\Contracts\Metrics\OperationalMetricsServiceInterface::class)->increment('payment_failed'); Log::channel('payments')->error('Payment transaction failed', [
    ...
    private function voucherPayment(Request $request)
    {
        ...
        if ($isSandbox && $voucherCode === 'TESTVOUCHER') {
            ...
            Log::channel('payments')->info('Sandbox demo voucher payment completed', [
                'transaction_id' => $merchantTransaction->trx_id,
        ...
        try {
            ...
            Log::channel('payments')->info('Voucher payment completed', [
                'transaction_id' => $merchantTransaction->trx_id,
                'voucher_code' => $voucherCode,
            ...
            notifyEvs('success', __('Pagamento com voucher concluído com sucesso.'));
            ...
            Log::channel('payments')->error('Voucher payment failed', [
                'transaction_id' => $merchantTransaction->trx_id,
                'voucher_code' => $voucherCode,
                'environment' => $environment->value,
                'is_sandbox' => $isSandbox,
    ...
    private function processMerchantTransaction(mixed $userWallet, mixed $merchantTransaction, mixed $user, mixed $merchant): RedirectResponse
    {
        ...
        try {
            ...
            app(\App\Contracts\Metrics\OperationalMetricsServiceInterface::class)->increment('checkout_completed'); Log::channel('payments')->info('Payment completed successfully', [
                'transaction_id' => $merchantTransaction->trx_id,
                'merchant_id' => $merchant->id,
                'user_id' => $user->id,
            ...
            Log::channel('payments')->error('Transaction process failed', [
                'transaction_id' => $merchantTransaction->trx_id,
                'environment' => $environment->value,
                'is_sandbox' => $isSandbox,
                'error' => $e->getMessage(),
