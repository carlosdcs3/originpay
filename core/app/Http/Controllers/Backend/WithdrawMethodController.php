...
class WithdrawMethodController extends BaseController
{
    ...
    private function prepareData(StoreUpdateRequest $request, ?WithdrawMethod $paymentMethod = null)
    {
        ...
        // Generate code if payment gateway and currency are available
        $code = ($paymentGateway && $request->filled('currency'))
            ? $paymentGateway->code . '_' . strtolower($validated['currency'])
            : ($validated['code'] ?? 'default-method-code'); // Fallback code
        ...
        $fields = $request->filled('fields')
            ...
            : [['name' => 'account_id', 'type' => 'text', 'validation' => 'required']];
        ...
        // Merge validated data, additional properties, and charge data
        return array_merge($validated, compact('logo', 'code', 'fields'), $chargeData, [
            'status' => $request->boolean('status'),
            'type'   => $request->input('type', MethodType::MANUAL->value), // Type is derived from input, defaults to manual
        ]);
    }
