...
class AuthController extends Controller implements HasMiddleware
{
    ...
    public function authenticate(AdminLoginRequest $request)
    {
        ...
        if ($request->isRecaptchaEnabled()) {
            ...
            if (! $request->verifyRecaptcha($token)) {
                notifyEvs('error', 'Falha ao verificar reCAPTCHA. Tente novamente.');
        ...
        if ($this->guard()->attempt($credentials, $remember)) {
            notifyEvs('success', 'Bem-vindo de volta! Você fez login com sucesso.');

            // Se a rota pretendida não for do admin, limpamos para evitar redirecionamento cruzado
        ...
        notifyEvs('error', 'E-mail ou senha inválidos. Tente novamente.');
    ...
    public function logout(Request $request)
    {
        ...
        // Só invalida a sessão global e regenera CSRF se o usuário não estiver logado
