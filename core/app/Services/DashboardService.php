...
class DashboardService
{
    ...
    public function getTransactionStatsByDay(User $user, array $trxTypes = [TrxType::DEPOSIT, TrxType::WITHDRAW], int $days = 7): array
    {
        ...
        $transactions = $user->transactions()
            ...
            ->get(['created_at', 'trx_type', 'currency', 'amount', 'status'])
            ->groupBy(fn ($transaction) => implode('|', [
                Carbon::parse($transaction->created_at)->format('D'),
                $transaction->trx_type instanceof TrxType ? $transaction->trx_type->value : $transaction->trx_type,
            ...
            ->map(function (Collection $group) {
                ...
                return (object) [
                    'label' => Carbon::parse($first->created_at)->format('D'),
                    'trx_type' => $first->trx_type instanceof TrxType ? $first->trx_type->value : $first->trx_type,
                    'currency' => $first->currency,
                    'success_total' => $group
                        ->filter(fn ($transaction) => $this->enumValue($transaction->status) === TrxStatus::COMPLETED->value)
                        ->sum('amount'),
                    'fail_total' => $group
                        ->filter(fn ($transaction) => in_array($this->enumValue($transaction->status), [
        ...
        foreach ($trxTypes as $type) {
            $result[$type->value] = collect($labels)->map(function ($label) use ($transactions, $type) {
                $filtered = $transactions
                    ->filter(fn ($row) => $this->enumValue($row->trx_type) === $type->value && $row->label === $label);
