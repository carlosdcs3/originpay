...
class ReferralContent extends Model
{
    ...
    public function getLocalizedPositiveGuidelines(?string $locale = null): array
    {
        ...
        if (is_array($guidelines) && isset($guidelines[$fallback])) {
            return $guidelines[$fallback];
        }

        if (is_array($guidelines) && isset($guidelines['en'])) {
            return $guidelines['en'];
        }

        return is_array($guidelines) ? (empty($guidelines) ? [] : reset($guidelines)) : [];
    }
