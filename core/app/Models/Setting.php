...
class Setting extends Model
{
    ...
    private static function getDefaultValue(string $key, ?string $section, mixed $default): mixed
    {
        ...
        if ($section) {
            return self::getDefaultValueForField($key, $section);
        }

        // Search through all sections in config if no section is provided
        $settingsConfig = config('settings') ?? [];
        foreach ($settingsConfig as $sec => $data) {
            if (isset($data['elements'])) {
                foreach ($data['elements'] as $element) {
                    if (isset($element['key']) && $element['key'] === $key) {
                        $val = $element['value'] ?? null;
                        // For select dropdowns that have [] as default value in config, return null instead
                        return (is_array($val) && empty($val)) ? null : $val;
                    }
                }
            }
        }
