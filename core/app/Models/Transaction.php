...
class Transaction extends Model
{
    ...
    protected $appends = ['currency_symbol', 'derived_status'];
    ...
    public static function boot()
    {
        ...
        static::deleting(function ($transaction) {
            throw new \Exception("Transações financeiras não podem ser excluídas fisicamente.");
        });

        static::updating(function ($transaction) {
            $immutableFields = ['amount', 'fee', 'net_amount', 'user_id', 'trx_type', 'provider', 'trx_reference'];
            foreach ($immutableFields as $field) {
                if ($transaction->isDirty($field)) {
                    throw new \Exception("O campo {$field} da transação é imutável e não pode ser alterado.");
                }
            }
            ...
            if ($transaction->isDirty('status')) {
                ...
                // Status can only change if it was PENDING
                if ($oldStatus !== TrxStatus::PENDING) {
                    throw new \Exception("Status da transação não pode ser alterado após sair de PENDING.");
                }

                // Status can only change to COMPLETED or FAILED
                if (!in_array($newStatus, [TrxStatus::COMPLETED, TrxStatus::FAILED])) {
                    throw new \Exception("A partir de PENDING, o status só pode mudar para COMPLETED ou FAILED.");
                }
            }
        });
    }
    ...
    public function getDerivedStatusAttribute(): string
    {
        ...
        if (!empty($this->trx_data['is_charged_back'])) {
            return 'charged_back';
        }

        if (!empty($this->trx_data['refund_amount']) && $this->trx_data['refund_amount'] > 0) {
            if ($this->trx_data['refund_amount'] >= $this->amount) {
                return 'refunded';
            }
            return 'partially_refunded';
        }

        if (!empty($this->trx_data['is_cancelled'])) {
            return 'cancelled';
        }
