...
class TemplateNotification extends Notification implements ShouldBroadcast, ShouldQueue
{
    ...
    public function via($notifiable): array
    {
        ...
        if (! $template) {
            Log::warning('Notification template not found; skipping notification dispatch.', [
                'identifier' => $this->identifier,
                'notifiable_type' => is_object($notifiable) ? $notifiable::class : gettype($notifiable),
    ...
    {
        return NotificationTemplate::where('identifier', $this->identifier)->first();
    }
