...
class ProcessCampaignRecipientJob implements ShouldQueue
{
    ...
    public function handle(TemplateEngine $engine, DeliveryService $deliveryService)
    {
        $executionId = null;

        $recipientMeta = ConnectCampaignRecipient::where('id', $this->recipientId)->select('id', 'merchant_id', 'channel', 'campaign_id', 'attempts', 'status')->first();
        ...
        DB::transaction(function () use ($engine, $deliveryService, &$executionId) {
            $recipient = ConnectCampaignRecipient::where('id', $this->recipientId)->lockForUpdate()->first();
