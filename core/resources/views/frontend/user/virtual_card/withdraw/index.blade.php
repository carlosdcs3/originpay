...
@section('title', __('Virtual Card Withdraw'))
@section('content')
	<div class="row">
		<div class="col-xl-7 mx-auto">
			<div class="v2-card">
				<div class="v2-card-header d-flex flex-wrap justify-content-between align-items-center">
					<h2 class="v2-card-title mb-0">{{ __('Withdraw from Virtual Card') }}</h6>
					<a class="v2-btn-secondary btn-sm" href="{{ route('user.transaction.index', ['type' => \App\Enums\TrxType::CARD_WITHDRAW]) }}">
				...
				<div class="v2-card-body">
					...
					<div class="alert alert-info d-flex align-items-center mb-3" role="alert">
						...
						<span>
							{{ __('Funds will be transferred to:') }}
							<strong>{{ $card->wallet->name ?? '-' }} @lang('Wallet')</strong>
							<span class="ms-2 small">({{ __('Available on Card:') }} {{ $card->wallet->currency->symbol.number_format($card->meta['balance'] ?? 0, 2) }})</span>
						</span>
					</div>
					<form action="{{ route('user.virtual-card.withdraw-store') }}" method="post" onsubmit="disableSubmitButton(this, '{{ __('Processing...') }}')">
						@csrf
						<input type="hidden" name="card_id" value="{{ $card->id ?? '' }}">
						<div class="v2-input-group mb-3">
							<label class="v2-label">{{ __('Amount') }}</label>
							<div class="input-group">
								<input type="text" step="0.01"
								       min="{{ $cardSettings->min_amount ?? '0.01' }}"
								       @if(!empty($cardSettings->max_amount)) max="{{ $cardSettings->max_amount }}" @endif
								       class="v2-input" name="amount"
							...
							<small class="text-muted">
								...
								@else
									{{ __('No withdraw limits. Any amount allowed.') }}
								@endif
							</small>
						</div>
						<button type="submit" class="v2-btn-primary w-100 submit-btn">
		...
		@include('frontend.user.virtual_card.withdraw.partials._summary')
	</div>
@endsection
