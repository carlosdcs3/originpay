...
<style>
    ...
    /* Segments & Pills */
    .op-segments { display: flex; background: var(--op-bg); border: 1px solid var(--op-border); border-radius: var(--op-radius-md); padding: 2px; user-select: none; }
    .op-segment { flex: 1; height: 30px; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 500; color: var(--op-text-muted); cursor: pointer; border-radius: var(--op-radius-sm); transition: var(--transition); outline: none; -webkit-tap-highlight-color: transparent; }
    ...
    #live-mock {
        --chk-bg: #111218;
        --chk-border: rgba(124,58,237,0.15);
        --chk-text: #F0F0F5;
        --chk-text-muted: #6E6E85;
        --chk-input-bg: #161820;
        --chk-input-border: rgba(255,255,255,0.07);
        --chk-input-text: #F0F0F5;
        --chk-tab-bg: #161820;
        --chk-tab-active-text: #F0F0F5;
        --chk-icon-filter: brightness(0) invert(1);
        --chk-primary: #7C3AED;
        --chk-primary-rgb: 124, 58, 237;
    }
    ...
    .chk-head {
        ...
        border-bottom: 1px solid var(--chk-input-border);
    }
    ...
    .chk-info { flex: 1; }
    .chk-title { font-size: 0.9rem; font-weight: 700; color: var(--chk-text); line-height: 1.3; }
    .chk-seller { font-size: 0.68rem; color: var(--chk-text-muted); margin-top: 2px; }
    .chk-amount-col { text-align: right; flex-shrink: 0; }
    .chk-price { font-size: 1.35rem; font-weight: 900; color: var(--chk-text); letter-spacing: -0.03em; }
    .chk-currency { font-size: 0.65rem; color: var(--chk-text-muted); text-align: right; margin-top: 2px; }
    .chk-badge {
        display: inline-flex; align-items: center; gap: 5px;
        padding: 3px 9px; border-radius: 999px;
        font-size: 0.62rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;
        background: rgba(var(--chk-primary-rgb), 0.15); color: #C4B5FD;
        border: 1px solid rgba(var(--chk-primary-rgb), 0.25);
        margin-top: 10px;
    }
    ...
    .chk-method-label { font-size: 0.6rem; font-weight: 700; color: var(--chk-text-muted); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px; }
    .chk-method-tabs { display: flex; gap: 6px; }
    .chk-method-tab {
        flex: 1; display: flex; flex-direction: column; align-items: center; gap: 4px;
        padding: 8px 4px; background: var(--chk-tab-bg);
        border: 1px solid var(--chk-input-border); border-radius: 8px;
        font-size: 0.62rem; font-weight: 600; color: var(--chk-text-muted);
        transition: background 0.3s, border-color 0.3s, color 0.3s;
    }
    .chk-method-tab.active {
        border-color: var(--chk-primary);
        background: rgba(var(--chk-primary-rgb), 0.1);
        color: var(--chk-tab-active-text);
        box-shadow: 0 0 0 2px rgba(var(--chk-primary-rgb), 0.12);
    }
    .chk-method-tab i, .chk-method-tab .pix-icon { font-size: 0.9rem; }
    .chk-method-tab .pix-icon {
    ...
    .chk-method-tab.active .pix-icon { opacity: 1; }
    ...
    .chk-field label { font-size: 0.58rem; font-weight: 600; color: var(--chk-text-muted); }
    .chk-fake-input {
        height: 32px; background: var(--chk-input-bg);
        border: 1px solid var(--chk-input-border); border-radius: 7px;
        display: flex; align-items: center; padding: 0 8px;
        gap: 6px; font-size: 0.65rem; color: var(--chk-text-muted);
        transition: background 0.3s, border-color 0.3s, color 0.3s;
    }
    .chk-fake-input i { font-size: 0.65rem; color: var(--chk-text-muted); }
    .chk-fake-input span { color: var(--chk-input-text); font-size: 0.7rem; }
    ...
    .chk-seal i { color: var(--chk-primary); font-size: 0.65rem; }
...
<!-- Main Form -->
<form id="pl-form" method="POST" action="{{ route('user.payment-links.store') }}" enctype="multipart/form-data" style="display: flex; flex-direction: column; flex: 1;">
    @csrf
    <input type="hidden" name="type" id="link-type-input" value="{{ $type }}">
    ...
    <div class="op-grid">
        ...
        <div style="display: flex; flex-direction: column; height: 100%;">
            ...
            <!-- Type Selector -->
            <div class="op-card" style="padding: 12px; margin-bottom: 16px;">
                <div class="op-label" style="margin-bottom: 8px;">Tipo de link</div>
                <div class="op-segments" style="height: 38px;">
                    <div id="btn-type-charge" class="op-segment {{ $type === 'charge' ? 'active' : '' }}">Pagamento Único</div>
                    <div id="btn-type-subscription" class="op-segment {{ $type === 'subscription' ? 'active' : '' }}">Assinatura</div>
                </div>
            </div>
            ...
            <div class="op-card">
                ...
                <!-- Recurring Fields -->
                <div id="recurring-fields" class="op-row" style="display: {{ $type === 'subscription' ? 'grid' : 'none' }}; margin-top: 12px;">
                    <div>
                        <div class="op-label">Cobrar a cada</div>
                        <select name="interval_count" class="op-input no-icon" style="padding-left: 12px;">
                    ...
                    <div>
                        <div class="op-label">Período</div>
                        <select name="interval" id="in-interval" class="op-input no-icon" style="padding-left: 12px;">
                            <option value="month" {{ old('interval') == 'month' ? 'selected' : '' }}>Mês(es)</option>
                            <option value="week" {{ old('interval') == 'week' ? 'selected' : '' }}>Semana(s)</option>
                            <option value="day" {{ old('interval') == 'day' ? 'selected' : '' }}>Dia(s)</option>
                            <option value="year" {{ old('interval') == 'year' ? 'selected' : '' }}>Ano(s)</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="op-card">
                ...
                <div class="op-row">
                    ...
                    <div id="expires-fields" style="display: {{ $type !== 'subscription' ? 'block' : 'none' }};">
                    ...
                    <div class="op-row">
                        ...
                        <div>
                            ...
                            <div style="display:flex; gap:8px; align-items:center;">
                                ...
                                <button type="button" id="btn-reset-color" class="op-btn-secondary" style="height:36px; padding:0 10px; font-size:0.85rem; border:1px solid var(--op-border); color: #6E6E85;" title="Redefinir cor padrão"><i class="fas fa-undo"></i></button>
                            </div>
                    ...
                    <div class="op-row">
                        ...
                        <div id="custom-bg-wrapper" style="display: {{ old('bg_theme') == 'custom' ? 'block' : 'none' }};">
                            ...
                            <div style="display:flex; gap:8px; align-items:center;">
                                ...
                                <button type="button" id="btn-reset-bg" class="op-btn-secondary" style="height:36px; padding:0 10px; font-size:0.85rem; border:1px solid var(--op-border); color: #6E6E85;" title="Redefinir cor padrão"><i class="fas fa-undo"></i></button>
                            </div>
                ...
                <div class="chk-form">
                    <div class="chk-form-grid" style="margin-top:10px;">
                        <div class="chk-field">
                            ...
                            <div class="chk-fake-input"><i class="fas fa-user"></i><span>Jo&atilde;o da Silva</span></div>
                        </div>
                        <div class="chk-field">
                            <label>E-mail</label>
                            <div class="chk-fake-input"><i class="fas fa-envelope"></i><span>joao@email.com</span></div>
                        </div>
                        <div class="chk-field">
                            <label>CPF / CNPJ</label>
                            <div class="chk-fake-input"><i class="fas fa-id-card"></i><span>000.000.000-00</span></div>
                        </div>
                        <div class="chk-field">
                            <label>Telefone (opcional)</label>
                            <div class="chk-fake-input"><i class="fas fa-phone"></i><span>(11) 99999-9999</span></div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    maskInput.addEventListener('input', function() {
        ...
        const btnAction = type === 'subscription' ? 'Assinar por R$ ' : 'Pagar R$ ';
        
        let intervalSuffix = '';
        if (type === 'subscription') {
            const inInterval = document.getElementById('in-interval');
            if (inInterval) {
                const iVal = inInterval.value;
                if (iVal === 'month') intervalSuffix = ' / mês';
                else if (iVal === 'week') intervalSuffix = ' / sem';
                else if (iVal === 'day') intervalSuffix = ' / dia';
                else if (iVal === 'year') intervalSuffix = ' / ano';
            }
        }
        
        if(realVal > 0) {
            prevPrice.innerText = 'R$ ' + this.value + intervalSuffix;
            prevBtnText.innerText = btnAction + this.value;
            prevPrice.style.opacity = '1';
        } else {
            prevPrice.innerText = 'R$ 0,00' + intervalSuffix;
            prevBtnText.innerText = btnAction + '0,00';
    ...
    const inInterval = document.getElementById('in-interval');
    if (inInterval) {
        inInterval.addEventListener('change', function() {
    ...
    // Type Switcher
    const btnTypeCharge = document.getElementById('btn-type-charge');
    const btnTypeSubscription = document.getElementById('btn-type-subscription');
    const recurringFields = document.getElementById('recurring-fields');
    const expiresFields = document.getElementById('expires-fields');
    const linkTypeInput = document.getElementById('link-type-input');

    function switchType(type) {
        linkTypeInput.value = type;
        if (type === 'subscription') {
            if(btnTypeSubscription) btnTypeSubscription.classList.add('active');
            if(btnTypeCharge) btnTypeCharge.classList.remove('active');
            if(recurringFields) recurringFields.style.display = 'grid';
            if(expiresFields) expiresFields.style.display = 'none';
        } else {
            if(btnTypeCharge) btnTypeCharge.classList.add('active');
            if(btnTypeSubscription) btnTypeSubscription.classList.remove('active');
            if(recurringFields) recurringFields.style.display = 'none';
            if(expiresFields) expiresFields.style.display = 'block';
        }
        maskInput.dispatchEvent(new Event('input'));
    }

    if (btnTypeCharge && btnTypeSubscription) {
        btnTypeCharge.addEventListener('click', () => switchType('charge'));
        btnTypeSubscription.addEventListener('click', () => switchType('subscription'));
    }
    ...
    document.querySelectorAll('.pm-checkbox').forEach(cb => {
        cb.addEventListener('change', syncMethods);
    });
    syncMethods();
    ...
    if(inExpires && inExpires.value && !['24h', '7d', ''].includes(document.querySelector('.op-pill.active')?.dataset.val)) {
        ...
        const customPill = document.querySelector('.op-pill[data-val="custom"]');
        if(customPill && customPill.classList.contains('active')) {
             if(customDateWrapper) customDateWrapper.style.display = 'block';
             if(inExpiresCustom) inExpiresCustom.value = inExpires.value.substring(0, 10);
        }
    }
    ...
    function updatePrimaryColor(hex) {
        ...
        if(mockContainer) {
            mockContainer.style.setProperty('--chk-primary', hex);
            
            // Calculate RGB for rgba() uses
            let r = parseInt(hex.substr(1, 2), 16) || 124;
            let g = parseInt(hex.substr(3, 2), 16) || 58;
            let b = parseInt(hex.substr(5, 2), 16) || 237;
            mockContainer.style.setProperty('--chk-primary-rgb', `${r}, ${g}, ${b}`);
        }
        document.getElementById('cust-status').innerText = 'Personalizado';
    }
    ...
    if(btnResetColor) {
        btnResetColor.addEventListener('click', function() {
            inColorPicker.value = '#7C3AED';
            inPrimaryColor.value = '#7C3AED';
    ...
    if(btnResetBg) {
        btnResetBg.addEventListener('click', function() {
            inBgPicker.value = '#0E0E11';
            inBgColor.value = '#0E0E11';
            inBgTheme.value = 'dark';
            updateBgTheme();
        });
    }

    // Set initial custom bg
    if(inBgTheme.value === 'custom') {
        applyCustomBg(inBgColor.value);
    } else {
        updateBgTheme();
    }
    ...
    function applyCustomBg(hex) {
        ...
        mockContainer.style.setProperty('--chk-bg', hex);
        if (isLight) {
            mockContainer.style.setProperty('--chk-border', 'rgba(0,0,0,0.08)');
            mockContainer.style.setProperty('--chk-text', '#0F0F1A');
            mockContainer.style.setProperty('--chk-text-muted', '#52525B');
            mockContainer.style.setProperty('--chk-input-bg', 'rgba(0,0,0,0.03)');
            mockContainer.style.setProperty('--chk-input-border', 'rgba(0,0,0,0.08)');
            mockContainer.style.setProperty('--chk-input-text', '#09090B');
            mockContainer.style.setProperty('--chk-tab-bg', 'rgba(0,0,0,0.02)');
            mockContainer.style.setProperty('--chk-tab-active-text', 'var(--chk-primary)');
            mockContainer.style.setProperty('--chk-icon-filter', 'invert(0) brightness(0)');
        } else {
            mockContainer.style.setProperty('--chk-border', 'rgba(124,58,237,0.15)');
            mockContainer.style.setProperty('--chk-text', '#F0F0F5');
            mockContainer.style.setProperty('--chk-text-muted', '#6E6E85');
            mockContainer.style.setProperty('--chk-input-bg', 'rgba(255,255,255,0.03)');
            mockContainer.style.setProperty('--chk-input-border', 'rgba(255,255,255,0.07)');
            mockContainer.style.setProperty('--chk-input-text', '#F0F0F5');
            mockContainer.style.setProperty('--chk-tab-bg', 'rgba(255,255,255,0.03)');
            mockContainer.style.setProperty('--chk-tab-active-text', '#F0F0F5');
            mockContainer.style.setProperty('--chk-icon-filter', 'brightness(0) invert(1)');
        }
    }
