    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="new-category-modal-label">{{ __('Add New Category') }}</h5>
                <button type="button" class="btn-close" data-coreui-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.support-ticket.category.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary"> <x-icon name="check" height="24" class="me-1" /> {{ __('Create Now') }}</button>
                    </div>
