            ...
            <div>
                <form method="POST" action="{{ route('admin.notifications.markAllAsRead') }}" class="d-inline">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fa fa-check-double me-1 me-1"></i> {{ __('Mark all as read') }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="card-body p-0">
            <div class="list-group list-group-flush">
                @forelse($notifications as $notification)
                    ...
                    <div class="list-group-item px-4 py-3 d-flex gap-3 align-items-start notification-item {{ $isUnread ? 'unread' : '' }}">
                        {{-- Avatar --}}
                        <div class="flex-shrink-0">
                            <img src="{{ asset($sender['avatar']) }}" alt="avatar" class="rounded-circle notification-avatar-img">
                        </div>
                        
                        {{-- Content --}}
                        <div class="flex-grow-1">
                            <a href="{{ $notification->data['action_link'] ?? 'javascript:void(0);' }}"
                               data-id="{{ $notification->id }}"
                               class="text-decoration-none read-notification">
