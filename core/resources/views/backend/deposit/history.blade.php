    ...
    <div class="card border-0 mb-4">
        <div class="card-header bg-white pt-3 pb-0 border-bottom-0">
            <ul class="nav nav-tabs border-bottom">
                <li class="nav-item">
                    <a class="nav-link {{ request('status', 'all') === 'all' ? 'active fw-bold' : 'text-muted' }}" href="{{ route('admin.deposit.history') }}">Todos</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === '2' ? 'active fw-bold' : 'text-muted' }}" href="{{ route('admin.deposit.history', ['status' => 2]) }}">Pendentes</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === '1' ? 'active fw-bold' : 'text-muted' }}" href="{{ route('admin.deposit.history', ['status' => 1]) }}">Aprovados</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === '3' ? 'active fw-bold' : 'text-muted' }}" href="{{ route('admin.deposit.history', ['status' => 3]) }}">Rejeitados</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
