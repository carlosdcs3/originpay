<script>
    $(document).ready(function () {
        ...
        let i = Date.now(); // Initialize field count uniquely
        ...
        function actionEvent(idName, isEdit = false) {
            ...
            // Reset charge inputs when modal is closed
            $modalForm.on('hidden.bs.modal', function () {
                // Clear charge values
                $modalForm.find('input[name="user_charge"]').val('');
                $modalForm.find('input[name="merchant_charge"]').val('');
                $modalForm.find('select[name="user_charge_type"]').val('');
                $modalForm.find('select[name="merchant_charge_type"]').val('');
            });
            ...
            $modalForm.on('click', '#add-new-field', function () {
                const formTemplate = `@include('backend.withdraw.method.partials._method_append_form_field', ['key' => '${i}'])`;
