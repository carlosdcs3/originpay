...
export default function () {
    const payload = JSON.stringify({
        ...
        currency: 'BRL',
        description: 'Load test sustained stress charge',
    });

    const res = http.post(`${BASE_URL}/api/charges`, payload, { headers: HEADERS });
    
    assertSuccess(res, 'Charge Creation Stress');
    
    sleep(0.5); // Slightly longer pause for sustained load compared to spike
}
